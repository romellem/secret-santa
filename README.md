...
This is a project I wanted to make to create an bare-bones "[secret santa][1]" web application,
...
- It was nice to have the project _immediately_ moving forward. Sometimes, when starting from scratch,
  you have a bit of writers block. Getting some quick momemtum felt good.
...
- ChatGPT often would edit the wrong file. Like it was say "Here are your changes to file `A`. Would you
  now like me to continue with file `B`?" And I'd say "Yes," and then it would start completely rewriting
  file `A` with `B`'s changes! When that would happen, I had to "expand" the correct file in the Canvas,
  and then give it the prompt again.
- I found it hard to continue my train of thought when I got to a point where I could run the code and
  saw that it was returning errors. If it was able to immediately fix it, it wouldn't be clear what
  the change was, and sometimes, a larger solution would make more sense once I dug into the code holistically.
...
- It wasn't insightful in ways I wanted it to be. For example, I used an [LZMA][3] library to create
  the hash params, but the library it fairly old and thus doesn't offer a `Promise`-based API. Making
...
- At one point, it really got stuck on the "[Hamilton path][4]" problem. One time I told it about a bug
  I was seeing (took a while to debug that), and when I told it how to fix it, it just
  ...
  window wouldn't get overloaded. This is really lame and unintuitive.
